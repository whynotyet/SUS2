...
Table \ref{tab:dist} provides statistics and psychometric properties of the original and reversed SUS, and an example (items 3, 6, 9, 10) of the alternative scale. They share similar distributional characteristics, but the SUS has higher reliability than the alternative scale in terms of Cronbach's $\alpha$. A factor analysis yields two eigenvalues greater than one for the SUS, consistent with previous work on its factor structure \cite{lewis2009factor}.
